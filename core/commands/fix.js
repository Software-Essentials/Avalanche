...
function fix() {
  ...
  const animation = setInterval(() => {
    ...
    process.stdout.write(`${CoreUtil.terminalPrefix()}\x1b[32m Diagnosing project ${dots}\x1b[0m`)
    process.stdout.cursorTo(0);
  }, 50);
  CoreUtil.rmdirSyncRecursive(`${projectPWD}/node_modules`);
  ...
  iProcess.on("exit", (code, signal) => {
    clearInterval(animation);
    process.stdout.clearLine();
    process.stdout.write(`\n${CoreUtil.terminalPrefix()}\x1b[32m Cleaned project, try now.\n\x1b[0m`)
    ready();
  });
}
